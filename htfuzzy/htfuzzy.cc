...
main(int ac, char **av)
{
    ...
    {
	std::cout << "htfuzzy: No algorithms specified\n";
	usage();
    }
	...
	if (worddb->OpenRead(wordFile) == OK)
	{
		...
		{
		    std::cout << "htfuzzy: words: " << count << '\n';
		    std::cout.flush();
		}
	    }
	    if (debug)
	    {
		std::cout << "htfuzzy: total words: " << count << "\n";
		std::cout << "htfuzzy: Writing index files...\n";
	    }
	...
	while ((fuzzy = (Fuzzy *) noWordAlgorithms.Get_Next()))
	{
	    if (debug)
	    {
		std::cout << "htfuzzy: Selected algorithm: " << fuzzy->getName()
		     << std::endl;
	    }
...
usage()
{
    std::cout << "usage: htfuzzy [-c configfile][-v] algorithm ...\n";

...
Endings::createDB(Configuration &config)
{
    ...
    if (debug)
	std::cout << "htfuzzy/endings: Reading rules\n";
	
    if (readRules(rules, config["endings_affix_file"]) == NOTOK)
	return NOTOK;

    if (debug)
	std::cout << "htfuzzy/endings: Creating databases\n";
...
Endings::createRoot(Dictionary &rules, char *word2root, char *root2word, char *dictFile)
{
	...
	{
	    std::cout << "htfuzzy/endings: words: " << count << '\n';
	    std::cout.flush();
	}
	count++;
	...
	*p++ = '\0';
	...
	if (debug > 1)
	    std::cout << "htfuzzy/endings: " << word << " --> " << words << std::endl;
		    ...
		    Dictionary &rules, char *word, char *suffixes)
{
	...
	for (int i = 0; i < suffixRules->Count(); i++)
	{
		...
		continue;
	    if (debug > 2)
		std::cout << "Applying regex '" << entry->expression << "' to " << word << std::endl;

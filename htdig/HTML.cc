...
HTML::do_tag(Retriever &retriever, String &tag)
{
    ...
    if (debug > 3)
	std::cout << "Tag: <" << tag << ">, matched " << which << std::endl;
	...
	case 0:		// "title"
	    ...
	    {
		if (debug)
		    std::cout << "More than one <title> tag in document!"
			 << " (possible search engine spamming)" << std::endl;
		break;
	    }
	    in_title = 1;
	    in_heading = 1;
	...
	case 2:		// "a"
	{
	  if (attrs["href"])
		...
		{
		  if (debug > 1)
		    std::cout << "Terminating previous <a href=...> tag,"
	...
	case 20:	// "meta"
	{
			 ...
			 && strlen(attrs["content"]) != 0)
		{
		   ...
		   if (debug > 1)
		     std::cout << "META Description: " << attrs["content"] << std::endl;

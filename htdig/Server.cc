...
{
    if (debug > 0)
	std::cout << std::endl << "New server: " << host << ", " << port << std::endl;
...
void Server::robotstxt(Document &doc)
{
    ...
    if (debug > 1)
	std::cout << "Parsing robots.txt file using myname = " << myname << "\n";
    ...
    {
	if (debug > 2)
	    std::cout << "Robots.txt line: " << line << std::endl;
	...
	{
	    if (debug > 1)
		std::cout << "Found 'user-agent' line: " << rest << std::endl;
	...
	{
	    if (debug > 1)
		std::cout << "Found 'disallow' line: " << rest << std::endl;
...
void Server::push(char *path, int hopcount, char *referer, int local)
{
    ...
    {
	if (debug > 1)
	    std::cout << "robots.txt: discarding '" << path <<
		"', which = " << which << ", length = " << length << std::endl;
	return;
    }
...
//	std::cout << "***** pushing '" << path << "' with '" << referer << "'\n";
}

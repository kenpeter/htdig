...
ExternalParser::parse(Retriever &retriever, URL &base)
{
    ...
    {
      if (debug)
	std::cout << "External parser error: Can't create temp file "
	     << (char *)path << std::endl;
      return;
    }
    ...
    {
      if (debug)
	std::cout << "External parser error: Can't create pipe!" << std::endl;
      unlink((char*)path);
      delete [] parsargs;
      return;
    }
    ...
    {
      if (debug)
	std::cout << "Fork Failure in ExternalParser" << std::endl;
      unlink((char*)path);
      delete [] parsargs;
      return;
    }
    ...
    {
      if (debug)
	std::cout << "Fdopen Failure in ExternalParser" << std::endl;
      unlink((char*)path);
      return;
    }
	...
	switch (*token1)
	{
		...
		if (token1 != NULL && token2 != NULL && token3 != NULL &&
		  ...
		  std::cerr<< "External parser error: expected word in line "<<line<<"\n" << " URL: " << base.get() << "\n";
		break;
		...
		if (token1 != NULL && token2 != NULL)
		{
		  ...
		  std::cerr<< "External parser error: expected URL in line "<<line<<"\n" << " URL: " << base.get() << "\n";
		break;
		...
		if (token1 != NULL)
		  ...
		  std::cerr<< "External parser error: expected title in line "<<line<<"\n" << " URL: " << base.get() << "\n";
		break;
		...
		if (token1 != NULL)
		  ...
		  std::cerr<< "External parser error: expected text in line "<<line<<"\n" << " URL: " << base.get() << "\n";
		break;
		...
		if (token1 != NULL)
		  ...
		  std::cerr<< "External parser error: expected anchor in line "<<line<<"\n" << " URL: " << base.get() << "\n";
		break;
		...
		if (token1 != NULL)
		  ...
		  std::cerr<< "External parser error: expected image URL in line "<<line<<"\n" << " URL: " << base.get() << "\n";
		break;
		...
		if (httpEquiv != NULL && name != NULL && content != NULL)
		{
		  ...
		  if (*name != '\0' && *content != '\0')
		  {
			...
			meta_dsc = meta_dsc.sub(0, max_meta_description_length).get();
		      if (debug > 1)
			std::cout << "META Description: " << content << std::endl;
		  ...
		  std::cerr<< "External parser error: expected metadata in line "<<line<<"\n" << " URL: " << base.get() << "\n";
		break;
	      }

	    default:
		  std::cerr<< "External parser error: unknown field in line "<<line<<"\n" << " URL: " << base.get() << "\n";
		break;
	}
    } // while(readLine)
    if (get_file)
	    ...
	    mystrncasecmp((char*)convertToType, "application/pdf", 15) != 0)
	{
		...
		std::cerr << "External parser error: no Content-Type given\n";
	    else
		std::cerr << "External parser error: can't parse Content-Type \""
		     << convertToType << "\"\n";
	    std::cerr << " URL: " << base.get() << "\n";
	}
	else
	{
	    char	buffer[2048];
	...
	else
	{
	    ...
	    parsable = plaintext;
	    if (debug)
		std::cout << "External parser error: \"" << contentType <<

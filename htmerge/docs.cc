...
convertDocs(char *doc_db, char *doc_index)
{
	...
	if (strlen(ref->DocHead()) == 0)
	  {
	    ...
	    db.Delete(url->get());
            if (verbose)
              std::cout << "Deleted, no excerpt: " << id.get() << "/"
                   << url->get() << std::endl;
	  }
	else if ((ref->DocState()) == Reference_noindex)
	  {
	    ...
	    db.Delete(url->get());
            if (verbose)
              std::cout << "Deleted, noindex: " << id.get() << "/"
                   << url->get() << std::endl;
	  }
	else if (remove_unused && discard_list.Exists(id))
	  {
	    ...
	    db.Delete(url->get());
            if (verbose)
              std::cout << "Deleted, invalid: " << id.get() << "/"
                   << url->get() << std::endl;
	  }
	else
	  {
	    ...
	    index->Put(id, coded_url, strlen(coded_url));
            if (verbose > 1)
              std::cout << "" << id.get() << "/" << url->get() << std::endl;

	    document_count++;
	    docdb_size += ref->DocSize();
	    if (verbose && document_count % 10 == 0)
	    {
		std::cout << "htmerge: " << document_count << '\n';
		std::cout.flush();
	    }
	  }
        delete ref;
    ...
    if (stats)
      {
	std::cout << "htmerge: Total documents: " << document_count << std::endl;
	std::cout << "htmerge: Total size of documents (in K): ";
	std::cout << docdb_size / 1024 << std::endl;
      }

	...
	if (old_ref)
	  {
		...
		if (verbose > 1)
		  {
		    std::cout << "htmerge: Duplicate, URL: " << url << " ignoring merging copy   \n";
		    std::cout.flush();
		  }
	      }
	    else
		...
		db.Add(*ref);
                if (verbose > 1)
                  {
                    std::cout << "htmerge: Duplicate, URL: ";
		    std::cout << url->get() << " ignoring destination copy   \n";
                    std::cout.flush();
                  }
	      }
	  }
	else
	  {
	    ...
	    if (verbose > 1)
	      {
		std::cout << "htmerge: Merged URL: " << url->get() << "    \n";
		std::cout.flush();
	      }
	  }
        delete ref;
	...
	if (db_dup_ids.Exists(sid))
	  {
	    if (verbose > 1)
	      {
		std::cout << "htmerge: Discarding duplicate " << word << " in doc #"
		     << sid << "     \n";
		std::cout.flush();
	      }
	    continue;
	...
	if (merge_dup_ids.Exists(sid))
	  {
	    if (verbose > 1)
	      {
		std::cout << "htmerge: Discarding merged duplicate " << word << " in doc #"
		     << sid << "     \n";
		std::cout.flush();
	      }
	    continue;

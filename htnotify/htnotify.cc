...
int main(int ac, char **av)
{
    ...
    {
      std::cerr << form("htnotify: Invalid url_part_aliases or common_url_parts: %s",
                   url_part_errors.get()) << std::endl;
      exit (1);
    }
...
void readPreAndPostamble(void)
{
    ...
    if (verbose > 1)
    {
        std::cout << "Preamble text:" << std::endl;
        std::cout << preambleText << std::endl << std::endl;
        std::cout << "Postamble text:" << std::endl;
        std::cout << postambleText << std::endl;
        std::cout << std::endl;
    }
}
...
void htnotify(DocumentRef &ref)
{
	...
	if (verbose > 2)
	{
	    std::cout << "Saw a date:" << std::endl;
	    std::cout << "Date:    " << date << std::endl;
	    std::cout << "URL:     " << ref.DocURL() << std::endl;
	    std::cout << "Subject: " << ref.DocSubject() << std::endl;
	...
	if (!parse_date(date, year, month, day))
	{
	    ...
	    if (verbose > 2)
	    {
		std::cout << "Malformed date: " << date << std::endl;
	    }
	...
	else
	{
	    ...
	    if (verbose > 2)
	    {
		std::cout << "htnotify: URL " << ref.DocURL()
		     << " (" << year+1900 << "-" << month+1
		     << "-" << day << ")" << std::endl;
	    }
	}
...
//      *cursor = '_'; // Set it to something harmless
    ...
    {   // just list the notifiable pages
        std::cout << std::endl;
        std::cout << "Notification required to " << to << std::endl;
        std::cout << listText;
    }
}
                 ...
                 String& to, String& listText, int singleSubject)
{
    ...
    if (verbose)
    {
        ...
        if (verbose > 1) { std::cout << listText; }
    }
...
void usage()
{
    std::cout << "usage: htnotify [-c configfile][-b db_base]\n";
    std::cout << "This program is part of ht://Dig " << VERSION << "\n\n";
    std::cout << "There can be any number or words.\n";
    std::cout << "Options:\n";
    std::cout << "\t-c configfile\n";
    std::cout << "\t\tUse the specified configuration file instead of the default.\n\n";
    std::cout << "\t-b db_base\n";
    std::cout << "\t\tSet the base path of the document database.\n";
    std::cout << "\t-v\n";
    std::cout << "\t\tIncrease the verbose level. Use two or three times for\n";
    std::cout << "\t\tmore output.\n";
    std::cout << "\t-n\n";
    std::cout << "\t\tDon't send any email, just list what has expired.\n";
    exit(0);
}
...
int parse_date(char *date, int &year, int &month, int &day)
{
	...
	yy += 1900;
    if (verbose > 2)
	std::cout << "Date used (y-m-d): " << yy << '-' << mm << '-' << dd << std::endl;

...
main(int ac, char **av)
{
    ...
    //if (debug > 2)
    //  std::cout << "Excerpt pattern: " << origPattern << "\n";
...
createLogicalWords(List &searchWords, String &logicalWords, String &wm)
{
    ...
    if (debug)
    {
	std::cerr << "LogicalWords: " << logicalWords << std::endl;
...
//			reportError("Syntax error");
	...
	if (ww->weight > 0 && !ww->isIgnore)
	{
	    ...
	    //
	    if (debug)
	      std::cerr << "Fuzzy on: " << ww->word << std::endl;
...
reportError(char *msg)
{
    ...
    std::cout << "<html><head><title>htsearch error</title></head>\n";
    std::cout << "<body bgcolor=\"#ffffff\">\n";
    std::cout << "<h1>ht://Dig error</h1>\n";
    std::cout << "<p>htsearch detected an error.  Please report this to the\n";
    std::cout << "webmaster of this site.  The error message is:</p>\n";
    std::cout << "<pre>\n" << msg << "\n</pre>\n</body></html>\n";
    exit(1);
}
...
void usage()
{
  std::cout << "usage: htsearch [-v][-d][-c configfile] [query_string]\n";
  std::cout << "This program is part of ht://Dig " << VERSION << "\n\n";
  std::cout << "Options:\n";
  std::cout << "\t-v -d\tVerbose mode.  This increases the verbosity of the\n";
  std::cout << "\t\tprogram.  Using more than 2 is probably only useful\n";
  std::cout << "\t\tfor debugging purposes.  The default verbose mode\n";
  std::cout << "\t\tgives a progress on what it is doing and where it is.\n\n";
  std::cout << "\t-c configfile\n";
  std::cout << "\t\tUse the specified configuration file instead on the\n";
  std::cout << "\t\tdefault.\n\n";
  std::cout << "\tquery_string\tA CGI-style query string can be given as a single\n";
  std::cout << "\t\targument, and is only used if the REQUEST_METHOD environment\n";
  std::cout << "\t\tvariable is not set.  If no query_string is given, and\n";
  std::cout << "\t\tREQUEST_METHOD is not set, htsearch will prompt for the query.\n\n";
  exit(0);
}
